...
(
    ...
    output  reg     [11:0]  read_addr       ,   //地址数据,辅助预充电操作,行、列地址,A12-A0,13位地址
    ...
    if(sys_rst_n == 1'b0)
        ...
        case(read_state)
            ...
            RD_ACTIVE:  //激活指令
                begin
                    read_cmd    <=  ACTIVE;
                    read_ba     <=  rd_addr[22:21];
                    read_addr   <=  rd_addr[20:9];
                end
            RD_READ:    //读操作指令
                begin
                    read_cmd    <=  READ;
                    read_ba     <=  rd_addr[22:21];
                    read_addr   <=  {3'b000,rd_addr[8:0]};
                end
            RD_DATA:    //突发传输终止指令
                begin
                    if(rdburst_end == 1'b1)
