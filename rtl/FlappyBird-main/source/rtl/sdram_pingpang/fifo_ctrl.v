...
always@(posedge sys_clk or negedge sys_rst_n)
    if(sys_rst_n == 1'b0)
        ...
        begin
            if(sdram_wr_addr[20:0] < (sdram_wr_e_addr - wr_burst_len))
                begin
                    bank_en     <=  bank_en;
                    bank_flag   <=  bank_flag;
                end
...
always@(posedge sys_clk or negedge sys_rst_n)
    if(sys_rst_n == 1'b0)
        ...
        begin
            if((pingpang_en ==1'b1) && (sdram_wr_addr[20:0] < (sdram_wr_e_addr - wr_burst_len)))
        ...
        begin
            if(bank_flag == 1'b0)    //切换Bank地址
                sdram_wr_addr   <=  {2'b00,sdram_wr_b_addr[20:0]};
            else
                sdram_wr_addr   <=  {2'b01,sdram_wr_b_addr[20:0]};
        end
    ...
    if(sys_rst_n == 1'b0)
        ...
        begin
            if(pingpang_en == 1'b1) //使用乒乓操作,读地址未达到末地址,读地址累加
                begin
                    if(sdram_rd_addr[20:0] < (sdram_rd_e_addr - rd_burst_len))
                        sdram_rd_addr   <=  sdram_rd_addr + rd_burst_len;
                    else
                        begin
                            if(bank_flag == 1'b0)    //切换Bank地址
                                sdram_rd_addr   <=  {2'b01,sdram_rd_b_addr[20:0]};
                            else
                                sdram_rd_addr   <=  {2'b00,sdram_rd_b_addr[20:0]};
                        end
                end
