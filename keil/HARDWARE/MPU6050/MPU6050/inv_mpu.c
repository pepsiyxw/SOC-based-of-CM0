...
u8 mpu_dmp_init(void)
{
	...
	{	 
		...
		if(res)return 2; 
		res=mpu_set_sample_rate(DEFAULT_MPU_HZ);	//设置采样率,DEFAULT_MPU_HZ
		if(res)return 3; 
		...
		if(res)return 6; 
		res=dmp_set_fifo_rate(DEFAULT_MPU_HZ);	//设置DMP输出速率(最大不超过200Hz)DEFAULT_MPU_HZ
		if(res)return 7;   
...
u8 mpu_dmp_get_data(float *pitch,float *roll,float *yaw)
{
	...
	unsigned char more;
	long quat[4];
		//OLED_ShowNum(80,50,dmp_read_fifo(gyro, accel, quat, &sensor_timestamp, &sensors,&more),1,12,1);
